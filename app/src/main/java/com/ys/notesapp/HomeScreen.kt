...
import com.ys.notesapp.ui.NoteViewModel
        ...
        LazyColumn(modifier = modifier.padding(top = 60.dp, bottom = 80.dp)) {
            items(notes) {
                NoteListItem(note = it) {
                    navController.navigate("$EDIT_NOTE/${it.id}/${it.noteTitle}/${it.noteDetails}")
                }
            }
        }
    }
...
private fun HomeScreenPreview() {
    NoteListItem(note = Note(5, "Test Title", "Hello this is a test note")) {}
}